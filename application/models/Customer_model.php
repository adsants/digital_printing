...
class Customer_model extends CI_Model {
	...
	function showData($where = null,$like = null,$order_by = null,$limit = null, $fromLimit=null){
		...
		return $this->db->get("m_customer",$limit,$fromLimit)->result();
	}
	...
	function getCount($where = null,$like = null,$order_by = null,$limit = null, $fromLimit=null){
		...
		return $this->db->get("m_customer",$limit,$fromLimit)->num_rows();
	}
	
	function getData($where){
		$this->db->select("*");		
		$this->db->where($where);		
		return $this->db->get("m_customer")->row();
	}
	
	
	function getPrimaryKeyMax(){
		$query = $this->db->query('select max(id_customer) as MAX from m_customer') ;	
		return $query->row();
	}
